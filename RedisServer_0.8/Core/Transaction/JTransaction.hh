...
class JTransaction : Noncopyable{
private:
public:
    ...
    void executeCommandIsMulti(const StringVec& stringVec, Jedis::JReply &reply);
...
private:
    void handle_packet(const std::vector<std::string> &packet, JReply &reply);
    void reset_status()
    {
      is_mult_flag_ = false;
