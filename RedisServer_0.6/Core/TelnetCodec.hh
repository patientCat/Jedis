...
: Noncopyable
{
  public:
    ...
    typedef std::function<void (const TcpConnectionPtr& conn, const std::vector<std::string>& stringVec, Timestamp recvTime)> StringVectorCallback;
  public:
    ...
    void onMessage(const TcpConnectionPtr& conn, Buffer *buffer, Timestamp receiveTime)
    {
      ...
      const char *end = buffer->find_crlf();
      if(!end)
        return;
