...
public:
    ...
    // 获取可读、可写起始位置处的指针
    const char* read_index_ptr() const {  return data() + readIndex_; }
    char * read_index_ptr() {   return data() + readIndex_;   }
    ...
    {
        const char*crlf = std::search(read_index_ptr(), write_index_ptr(), kCrlf, kCrlf + 2);
    ...
    {
        const void*eof = memchr(read_index_ptr(), '\n', readable_size());
    ...
    {
        std::string result(read_index_ptr(), len);
        retrieve(len);
        return result;
    }
    ...
    void prepend(const void* data, size_t len)
    {
        ...
        std::copy(d, d+len, read_index_ptr());
    }
