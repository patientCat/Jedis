...
class JPubSubDict
{
  public:
    ...
    void publish(const string& topic_name, const string& content, Timestamp time_stamp)
    {
      // 当前不管正则处理
    ...
    // 订阅
    void subscribe(std::string topic_name, const TcpConnectionPtr & conn)
    {
      if(!hashmap_.count(topic_name))
      {
        // 新建一个topic
    ...
    // 退订
    void unSubscribe(std::string topic_name, const TcpConnectionPtr & conn)
    {
      auto &topic = hashmap_[topic_name];
      if(!topic->has_conn(conn))
      {
  ...
  private:
    std::unordered_map<std::string, std::unique_ptr<Topic>> hashmap_;
};
